  ...
  async run(message, args) {
    if (!message.member.permissions.has(PermissionFlagsBits.ManageMessages)) {
      return message.reply({
        content: "You don't have the required permission (`Manage Messages`).",
      });
    }
    const limit = args[0];
    if (!limit) {
      return message.reply({
        content: "You need to specify a number of messages !",
      });
    }
    const filter = args[1];
    if (!filter) {
      return message.reply({
        content:
          "You need to specify a filer: `[link|invite|bots|you|me|upload|user:user]` !",
      });
    }
    ...
    message
      .reply({
        content: `Successfully deleted \`${messages.length}\` messages from \`${limit}\`.`,
      })
      .then(async () => {
