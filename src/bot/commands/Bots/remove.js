...
module.exports = class extends Command {
  ...
  async run(message, args) {
    ...
    if (!user || !user.bot)
      return message.channel.send("You didn't ping a bot to remove.");
    ...
    message.reply({ embeds: [e] }).then(async (m) => {
      ...
      if (mainAnswer.size) {
        const answerMessage = mainAnswer.first();
        ...
        if (!message.client.users.cache.find((u) => u.id === bot.botid).bot)
          return;
        try {
