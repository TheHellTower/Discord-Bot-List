...
module.exports = class extends Command {
  ...
  async run(message, args) {
	...
	if(command) {
        ...
        this.client.commands.get(this.client.aliases.get(command.toLowerCase()));
	}

	if(command){
		return message.reply({content: `\`\`\`asciidoc\n= ${command.name} =\n> Description: ${command.description}\n> Usage: ${command.usage}\n\`\`\``})
	} else {
		var commands = await this._fetchCommands(message);
		const e = new EmbedBuilder()
		.setTitle("Commands");
		for (const [category, list] of commands) {
			var commandsList = "";
			list.forEach((command) => {commandsList += `\`${command.name}\`: \`${command.description}\` \n`})
			e.addFields(
				{name: `${category} Commands`, value: `${commandsList}`}
			)
		}
